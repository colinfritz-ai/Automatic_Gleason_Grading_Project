...
PATH_TO_REPO = '/Users/colinfritz/Desktop/my_repos/Automatic_Gleason_Grading_Project'
TRAIN_CSV_PATH = PATH_TO_REPO + '/train_test_csv_files/train.csv'
label_strings_array = create_label_strings(TRAIN_CSV_PATH)
image_filepaths_array = create_image_filepath_strings(TRAIN_CSV_PATH)

#parsing the lists into train and validation sets
train_label_strings = label_strings_array[0:int(len(label_strings_array)*0.8)]
train_image_filepaths = image_filepaths_array[0:int(len(label_strings_array)*0.8)]
validation_label_strings = label_strings_array[int(len(label_strings_array)*0.8):]
validation_image_file_paths = image_filepaths_array[int(len(label_strings_array)*0.8):]

#creating validation dataset with (image, label) elements as numpy
#arrays from the filepaths and strings in the original lists
validation_dataset = tf.data.Dataset.from_tensor_slices((validation_image_file_paths,validation_label_strings)) # pylint: disable=line-too-long
...
#creating train dataset with (image, label) elements as numpy arrays from
#the filepaths and strings in the original lists
...
ISUP_GRADING_MODEL.compile(loss = 'categorical_crossentropy',
    optimizer = 'SGD' , metrics =["accuracy"])
ISUP_GRADING_MODEL.fit(x=batched_train_dataset, epochs = 100 , verbose=1,
    validation_data = batched_validation_dataset, steps_per_epoch = 1, validation_steps = 1)
