...
# 	shards = len(train)//shard_size
# 	if len(train)%shard_size != 0:
# 		shards = shards + 1
# 	file_count=0
# 	for shard in range(shards):
# 		filename = "/Volumes/external_1tb/Automated_Gleason_Grading_Project_Resources/Resized_Datasets/train_" + str(shard)
# 		with tf.io.TFRecordWriter(filename) as out_file:
# 			for file in range(shard_size):
# 				if file_count <= (len(filenames)-1):
# 					image = preparer.resize_and_encode(filenames[file_count])
# 					label = preparer.ohe_label(labels[file_count])
...
# 	shards = len(validation)//shard_size
# 	if len(validation)%shard_size != 0:
# 		shards = shards + 1
# 	file_count=0
# 	for shard in range(shards):
# 		filename = "/Volumes/external_1tb/Automated_Gleason_Grading_Project_Resources/Resized_Datasets/validation_" + str(shard)
# 		with tf.io.TFRecordWriter(filename) as out_file:
# 			for file in range(shard_size):
# 				if file_count <= (len(filenames)-1):
# 					image = preparer.resize_and_encode(filenames[file_count])
...
shards = len(test)//shard_size
if len(test)%shard_size != 0:
	shards = shards + 1
file_count=0
for shard in range(shards):
	filename = "/Volumes/external_1tb/Automated_Gleason_Grading_Project_Resources/Resized_Datasets/test_" + str(shard)
	with tf.io.TFRecordWriter(filename) as out_file:
		for file in range(shard_size):
			if file_count <= (len(filenames)-1):
				image = preparer.resize_and_encode(filenames[file_count])
